...
class SubtitleFrameExtractor:
    ...
    def get_subtitles_with_one_frame_representing_each_subtitle(
    ...
    def _get_subtitles_with_defined_frames(
            self, subtitles: List[Subtitle]
    ) -> List[Subtitle]:
        ...
        for pos, sub in enumerate(subtitles):
            ...
            middle = ((sub.time_start + sub.time_end) // 2)
            middle_sec_from_subs = middle.total_seconds()
            subs_with_frames[pos].frame = Frame(
                self._video,
